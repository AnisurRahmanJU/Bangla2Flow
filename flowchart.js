...
function banglaToJS(code){
  return bnNumberToEn(code)
    ...
    .replace(/\bধ্রুবক\b/g,"const")
    .replace(/\bদেখাও\b/g,"console.log")
    .replace(/\bনাও\b/g,"prompt")
    .replace(/\bনাও\b/g,"Number(prompt)")
    .replace(/\bযদি\b/g,"if")
    .replace(/\bনাহলে\b/g,"else")
    .replace(/\bযতক্ষণ\b/g,"while")
    .replace(/\bলুপ\b/g,"for")
    .replace(/\bফাংশন\b/g,"function")
    .replace(/\bফেরত\b/g,"return")
    .replace(/\bএবং\b/g,"&&")
    .replace(/\bঅথবা\b/g,"||")
    .replace(/\bসত্য\b/g,"true")
    .replace(/\bমিথ্যা\b/g,"false")
    .replace(/\bসুইচ\b/g,"switch")
    .replace(/\bকেস\b/g,"case")
    .replace(/\bডিফল্ট\b/g,"default")
    .replace(/\bথামো\b/g,"break")
    .replace(/\bবাদ\b/g,"continue")
    .replace(/\bচেষ্টা\b/g,"try")
    .replace(/\bধরো\b/g,"catch")
    .replace(/\bশেষ\b/g,"finally")
    .replace(/\bছোড়ো\b/g,"throw")
    .replace(/\bদৈর্ঘ্য\b/g, "length")
    .replace(/\bনাল\b/g, "NULL")
    .replace(/\bপ্রতিটি\b/g,"for_of")
    .replace(/\bপ্রতিটি_ইন\b/g,"for_in")
    .replace(/\bরাখো\b/g,"push")
    .replace(/\bসরাও\b/g,"pop")
    .replace(/\bঅংশ\b/g,"slice")
    .replace(/\bবড়হাতেরঅক্ষর\b/g,"toUpperCase")
    .replace(/\bছোটহাতেরঅক্ষর\b/g,"toLowerCase")
    .replace (/\bউপস্ট্রিং\b/g, "substr")
